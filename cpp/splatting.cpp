    ...
    for (auto c = 0; c < C; c++) {
      for (auto y = 0; y < Y; y++) {
        for (auto x = 0; x < X; x++) {
          ...
          const scalar_t northwest = ((scalar_t)(southeastX)-outputX) *
              ((scalar_t)(southeastY)-outputY);
          const scalar_t northeast = (outputX - (scalar_t)(southwestX)) *
              ((scalar_t)(southwestY)-outputY);
          const scalar_t southwest = ((scalar_t)(northeastX)-outputX) *
              (outputY - (scalar_t)(northeastY));
          const scalar_t southeast = (outputX - (scalar_t)(northwestX)) *
      ...
      for (auto y = 0; y < Y; y++) {
        for (auto x = 0; x < X; x++) {
          ...
          if ((northwestX >= 0) && (northwestX < X) && (northwestY >= 0) &&
            ...
            grad_flow_a[n][0][y][x] += frame_here * grad_output_here *
                ((scalar_t)(-1.0)) * ((scalar_t)(southeastY)-outputY);
            grad_flow_a[n][1][y][x] += frame_here * grad_output_here *
                ((scalar_t)(southeastX)-outputX) * ((scalar_t)(-1.0));
          }
          if ((northeastX >= 0) && (northeastX < X) && (northeastY >= 0) &&
              (northeastY < Y)) {
            const scalar_t grad_output_here =
            ...
            grad_flow_a[n][0][y][x] += frame_here * grad_output_here *
                ((scalar_t)(+1.0)) * ((scalar_t)(southwestY)-outputY);
            grad_flow_a[n][1][y][x] += frame_here * grad_output_here *
                (outputX - (scalar_t)(southwestX)) * ((scalar_t)(-1.0));
          }
          if ((southwestX >= 0) && (southwestX < X) && (southwestY >= 0) &&
              (southwestY < Y)) {
            const scalar_t grad_output_here =
            ...
            grad_flow_a[n][0][y][x] += frame_here * grad_output_here *
                ((scalar_t)(-1.0)) * (outputY - (scalar_t)(northeastY));
            grad_flow_a[n][1][y][x] += frame_here * grad_output_here *
                ((scalar_t)(northeastX)-outputX) * ((scalar_t)(+1.0));
          }
          if ((southeastX >= 0) && (southeastX < X) && (southeastY >= 0) &&
              (southeastY < Y)) {
            const scalar_t grad_output_here =
            ...
            grad_flow_a[n][0][y][x] += frame_here * grad_output_here *
                ((scalar_t)(+1.0)) * (outputY - (scalar_t)(northwestY));
            grad_flow_a[n][1][y][x] += frame_here * grad_output_here *
                (outputX - (scalar_t)(northwestX)) * ((scalar_t)(+1.0));
          }
        }
      }
    }
...
void splatting_forward_cpu(
  ...
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      frame.scalar_type(), "splatting_forward_cpu", [&] {
...
void splatting_backward_cpu(
    ...
    torch::Tensor grad_flow) {
  AT_DISPATCH_FLOATING_TYPES_AND_HALF(
      frame.scalar_type(), "splatting_backward_cpu", [&] {
        splatting_backward_cpu_impl<scalar_t>(
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def(
      ...
      &splatting_forward_cpu,
      "splatting forward (CPU)");
  m.def(
      "splatting_backward_cpu",
      &splatting_backward_cpu,
      "splatting backward (CPU)");
}
