...
__global__ void splatting_forward_cuda_kernel(
    const torch::PackedTensorAccessor64<scalar_t, 4, torch::RestrictPtrTraits> frame_a,
    const torch::PackedTensorAccessor64<scalar_t, 4, torch::RestrictPtrTraits> flow_a,
    torch::PackedTensorAccessor64<scalar_t, 4, torch::RestrictPtrTraits> output_a
) {
    ...
    CUDA_KERNEL_LOOP(index, total_step) {
        ...
        if ((northwestX >= 0) && (northwestX < X) && (northwestY >= 0) && (northwestY < Y)) {
            gpuAtomicAdd(&(output_a[n][c][northwestY][northwestX]), northwest * frame_a[n][c][y][x]);
        }
        if ((northeastX >= 0) && (northeastX < X) && (northeastY >= 0) && (northeastY < Y)) {
            gpuAtomicAdd(&(output_a[n][c][northeastY][northeastX]), northeast * frame_a[n][c][y][x]);
        }
        if ((southwestX >= 0) && (southwestX < X) && (southwestY >= 0) && (southwestY < Y)) {
            gpuAtomicAdd(&(output_a[n][c][southwestY][southwestX]), southwest * frame_a[n][c][y][x]);
        }
        if ((southeastX >= 0) && (southeastX < X) && (southeastY >= 0) && (southeastY < Y)) {
            gpuAtomicAdd(&(output_a[n][c][southeastY][southeastX]), southeast * frame_a[n][c][y][x]);
        }
    }
}
...
__global__ void splatting_backward_cuda_kernel(
    const torch::PackedTensorAccessor64<scalar_t, 4, torch::RestrictPtrTraits> frame_a,
    const torch::PackedTensorAccessor64<scalar_t, 4, torch::RestrictPtrTraits> flow_a,
    const torch::PackedTensorAccessor64<scalar_t, 4, torch::RestrictPtrTraits> grad_output_a,
    torch::PackedTensorAccessor64<scalar_t, 4, torch::RestrictPtrTraits> grad_frame_a,
    torch::PackedTensorAccessor64<scalar_t, 4, torch::RestrictPtrTraits> grad_flow_a
) {
    const size_t N = frame_a.size(0);
    const size_t C = frame_a.size(1);
    const size_t Y = frame_a.size(2);
    ...
    CUDA_KERNEL_LOOP(index, total_step) {
        ...
        if ((northwestX >= 0) && (northwestX < X) && (northwestY >= 0) && (northwestY < Y)) {
            ...
            gpuAtomicAdd(&(grad_flow_a[n][0][y][x]), frame_here * grad_output_here * ((scalar_t) (-1.0)) * ((scalar_t) (southeastY) - outputY   ));
            gpuAtomicAdd(&(grad_flow_a[n][1][y][x]), frame_here * grad_output_here * ((scalar_t) (southeastX) - outputX   ) * ((scalar_t) (-1.0)));
        }
        if ((northeastX >= 0) && (northeastX < X) && (northeastY >= 0) && (northeastY < Y)) {
            ...
            gpuAtomicAdd(&(grad_flow_a[n][0][y][x]), frame_here * grad_output_here * ((scalar_t) (+1.0)) * ((scalar_t) (southwestY) - outputY   ));
            gpuAtomicAdd(&(grad_flow_a[n][1][y][x]), frame_here * grad_output_here * (outputX    - (scalar_t) (southwestX)) * ((scalar_t) (-1.0)));
        }
        if ((southwestX >= 0) && (southwestX < X) && (southwestY >= 0) && (southwestY < Y)) {
            ...
            gpuAtomicAdd(&(grad_flow_a[n][0][y][x]), frame_here * grad_output_here * ((scalar_t) (-1.0)) * (outputY    - (scalar_t) (northeastY)));
            gpuAtomicAdd(&(grad_flow_a[n][1][y][x]), frame_here * grad_output_here * ((scalar_t) (northeastX) - outputX   ) * ((scalar_t) (+1.0)));
        }
        if ((southeastX >= 0) && (southeastX < X) && (southeastY >= 0) && (southeastY < Y)) {
            ...
            gpuAtomicAdd(&(grad_flow_a[n][0][y][x]), frame_here * grad_output_here * ((scalar_t) (+1.0)) * (outputY    - (scalar_t) (northwestY)));
            gpuAtomicAdd(&(grad_flow_a[n][1][y][x]), frame_here * grad_output_here * (outputX    - (scalar_t) (northwestX)) * ((scalar_t) (+1.0)));
        }
    }
}
...
void splatting_forward_cuda_impl(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        frame.type(),
        "splatting_forward_cuda",
        [&] {
            splatting_forward_cuda_kernel<scalar_t><<<GET_BLOCKS(num_steps), CUDA_NUM_THREADS>>>(
...
void splatting_backward_cuda_impl(
    ...
    AT_DISPATCH_FLOATING_TYPES_AND_HALF(
        frame.type(),
        "splatting_backward_cuda",
        [&] {
            splatting_backward_cuda_kernel<scalar_t><<<GET_BLOCKS(num_steps), CUDA_NUM_THREADS>>>(
                frame.packed_accessor64<scalar_t, 4, torch::RestrictPtrTraits>(),
                flow.packed_accessor64<scalar_t, 4, torch::RestrictPtrTraits>(),
                grad_output.packed_accessor64<scalar_t, 4, torch::RestrictPtrTraits>(),
                grad_frame.packed_accessor64<scalar_t, 4, torch::RestrictPtrTraits>(),
                grad_flow.packed_accessor64<scalar_t, 4, torch::RestrictPtrTraits>()
            );
        }
    );
}
